...
class Pengaturan extends MX_Controller{
	...
	function index(){
		if($this->session->userdata('login')===TRUE){
			$datam['data']		=$this->pengaturan_model->get_userinfo($this->session->userdata('userid'));
	...
	function simpanprofil(){
            ...
            if ($this->form_validation->run($this) == TRUE) {
				...
				$this->pengaturan_model->update_info($data, $userid);
				...
				$query=null;
					if (empty($databank['idbank'])) {
						...
						$query=$this->pengaturan_model->insert_bank_account($databank);
					}
					else{
